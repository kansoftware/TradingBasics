...
TPriceSeries _TrueRange(const TBarSeries &aBars) {
  ...
  if (not aBars.empty()) {
    ...
    lResult.emplace_back(lPriorBar.DateTime, lPriorBar.High - lPriorBar.Low,
                         lPriorBar.Volume);
    ...
    for (size_t i = 1; i < aBars.size(); ++i) {
      ...
      lResult.emplace_back(lCurrentBar.DateTime, lHigh - lLow,
                           lCurrentBar.Volume);
...
//------------------------------------------------------------------------------------------
TPriceSeries _SimpleMA(const TPriceSeries &aPrices, const int aPeriod,
                       const size_t aLag) {
...
//------------------------------------------------------------------------------------------
TPriceSeries _ExponentMA(const TPriceSeries &aPrices, const int aPeriod,
                         const size_t aLag) {
  ...
  if (lResultSize > lPeriod) {
    ...
    lResult[lPeriod + aLag - 1] =
        TSimpleTick{aPrices[lPeriod + aLag - 1].DateTime, lExpPrice, 1.0};
...
//------------------------------------------------------------------------------------------
TPriceSeries _SmoothedMA(const TPriceSeries &aPrices, const int aPeriod,
                         const size_t aLag) {
  ...
  if (lResultSize > lPeriod + aLag) {
    ...
    for (size_t i = lPeriod + aLag; i < lResultSize; ++i) {
      lExpPrice = (aPrices[i].Price + lExpPrice * ToDouble(lPeriod - 1)) /
                  ToDouble(lPeriod);
      lResult[i] = TSimpleTick{aPrices[i].DateTime, lExpPrice, 1.0};
    }
...
bool _DM(const TBarSeries &aBars, TPriceSeries &aoDMp, TPriceSeries &aoDMn) {
  ...
  TSimpleTick lTick{aBars.begin()->DateTime, 0.0, aBars.begin()->Volume};
  lDMp[0] = lTick;
  lDMn[0] = lTick;
...
//------------------------------------------------------------------------------------------
bool _DI(const TBarSeries &aBars, const int aPeriod, TPriceSeries &aoDMIp,
         TPriceSeries &aoDMIn) {
  if (aPeriod < 2) {
    return false;
  }
  ...
  if (not _DM(aBars, lDMp, lDMn)) {
    return false;
  }

  const TPriceSeries lTR(_TrueRange(aBars));

  if (aBars.size() != lDMp.size() or lDMp.size() != lDMn.size() or
      lDMn.size() != lTR.size()) {
    return false;
  }
  ...
  for (size_t i = lPeriod; i < aBars.size(); ++i) {
    lDMSump =
        (lDMSump * ToDouble(lPeriod - 1) / ToDouble(lPeriod) + lDMp[i].Price);
    lTRSum =
        (lTRSum * ToDouble(lPeriod - 1) / ToDouble(lPeriod) + lTR[i].Price);
    const double lDMIp =
        (isPositiveValue(lTRSum) ? (100.0 * lDMSump / lTRSum) : 0.0);
    TSimpleTick lTick{aBars[i].DateTime, lDMIp, aBars[i].Volume};
    aoDMIp[i] = lTick;

    lDMSumn =
        (lDMSumn * ToDouble(lPeriod - 1) / ToDouble(lPeriod) + lDMn[i].Price);
    const double lDMIn =
        (isPositiveValue(lTRSum) ? (100.0 * lDMSumn / lTRSum) : 0.0);
    lTick.Price = lDMIn;
    aoDMIn[i] = lTick;
  }
...
TPriceSeries _ADX(const TBarSeries &aBars, const int aPeriod) {
  ...
  for (size_t i = ToSize_t(aPeriod); i < aBars.size(); ++i) {
    ...
    lDX[i].Price = std::fabs(100.0 * (lDMIp[i].Price - lDMIn[i].Price)) /
                   (lDMIp[i].Price + lDMIn[i].Price);
  }
...
//------------------------------------------------------------------------------------------
TPriceSeries _ParabolicSar(const TBarSeries &aBars, const double aAf,
                           const double aMaxAf) {
  ...
  const TSimpleTick lFirstTick{lFirstBar.DateTime, lFirstBar.Low, 1.0};
  ...
  for (size_t i = 1; i < aBars.size(); ++i) {
    ...
    if (lCurrentSide == TDealSide::Buy) {
      ...
      const TSimpleTick lTick{lCurrentBar.DateTime, lSar, 1.0};
      lResult[i] = lTick;
      continue;
    }
...
//------------------------------------------------------------------------------------------
TPriceSeries _MACD(const TPriceSeries &aPrices, const int aFastPeriod,
                   const int aSlowPeriod, const int aSmoothPeriod) {
  ...
  const size_t lMaxPeriod =
      ToSize_t((aSlowPeriod > aFastPeriod) ? aSlowPeriod : aFastPeriod);
  ...
  for (size_t i = lMaxPeriod - 1; i < lResultSize; ++i) {
    lDifference[i] =
        TSimpleTick{aPrices[i].DateTime,
                    (lFastExponentMA[i].Price - lSlowExponentMA[i].Price), 1.0};
  }

  const TPriceSeries lResult(
      _ExponentMA(lDifference, aSmoothPeriod, lMaxPeriod - 1));
  ...
  for (size_t i = 1; i < aBars.size(); ++i) {
    ...
    if (lSide == TDealSide::Buy) {
      ...
      if (aBars[i].High > lCurrHead) {
        ...
        lResult[i] =
            TSimpleTick{aBars[i].DateTime, lCurrHead, lSideToValue(lSide)};
        lResult[lLastHeadindex] = TSimpleTick{aBars[lLastHeadindex].DateTime,
                                              gFlatValue, lSideToValue(lSide)};
        lLastHeadindex = i;
        ...
        lLastHeadindex = i;
        lResult[lLastHeadindex] = TSimpleTick{aBars[lLastHeadindex].DateTime,
                                              lCurrHead, lSideToValue(lSide)};

      } else {
        lResult[i] =
            TSimpleTick{aBars[i].DateTime, gFlatValue, lSideToValue(lSide)};
      }
...
//------------------------------------------------------------------------------------------
bool _Forecasting(const TPriceSeries &aPrices, const size_t aForecastPeriod,
                  const double aConfidenceIntervals,
                  TPriceSeries &aoUpperBorder, TPriceSeries &aoForecast,
  ...
  for (size_t i = 0; i < aForecastPeriod; ++i) {
    const TSimpleTick lEmptyTick{aPrices[i].DateTime, GetBadPrice(), 0.0};
  ...
  for (size_t i = aForecastPeriod; i < lDataSize; ++i) {
    const TDoubles lDataF(std::next(lData.begin(), ToInt(i - aForecastPeriod)),
                          std::next(lData.begin(), ToInt(i)));
    const TForecastResult lResult(
        Forecast(lDataF, lParams, aConfidenceIntervals));

    if (IsValidPrice(lResult.forecast)) {
      TSimpleTick lTick{aPrices[i].DateTime, lResult.forecast, 1.0};
      aoForecast[i] = lTick;
...
bool _RollMinMax(const TBarSeries &aBars, const int aPeriod,
                 TPriceSeries &aoMin, TPriceSeries &aoMax, const bool aTouch) {
  ...
  for (size_t i = lPeriod; i < lDataSize; ++i) {
    ...
    aoMin.emplace_back(
        lBar.DateTime, lRange.first,
        (IsEqual(lRange.first, lBar.Low) ? 1.0 : (aTouch ? 0.0 : 1.0)));
    aoMax.emplace_back(
        lBar.DateTime, lRange.second,
        (IsEqual(lRange.second, lBar.High) ? 1.0 : (aTouch ? 0.0 : 1.0)));
  }

  if (aTouch) {
    for (size_t i = lPeriod; i < lDataSize; ++i) {
      aoMax[i].Volume = (isPositiveValue(aoMax[i].Volume) and
                         IsEqual(aoMax[i].Price, aoMax[i - 1].Price))
                            ? (aoMax[i].Volume + aoMax[i - 1].Volume)
                            : 0.0;
      aoMin[i].Volume = (isPositiveValue(aoMin[i].Volume) and
                         IsEqual(aoMin[i].Price, aoMin[i - 1].Price))
                            ? (aoMin[i].Volume + aoMin[i - 1].Volume)
                            : 0.0;
    }
  }
...
bool _RollMinMax_old(const TBarSeries &aBars, const int aPeriod,
                     TPriceSeries &aoMin, TPriceSeries &aoMax,
                     const bool aTouch) {
  ...
  for (size_t i = 0; i < lPeriod; ++i) {
    ...
    const TSimpleTick lEmptyTick{aBars[i].DateTime, GetBadPrice(), 0.0};
  ...
  for (size_t i = lPeriod; i < lDataSize; ++i) {
    ...
    TSimpleTick lTick{aBars[i].DateTime, lRollerLow.GetValue().min,
                      (IsEqual(lRollerLow.GetValue().min, aBars[i].Low)
                           ? 1.0
                           : (aTouch ? 0.0 : 1.0))};
    ...
    lTick.Volume = (IsEqual(lRollerHigh.GetValue().max, aBars[i].High)
                        ? 1.0
                        : (aTouch ? 0.0 : 1.0));
    aoMax[i] = lTick;
  }
...
//------------------------------------------------------------------------------------------
bool _SupportRessistance(const TBarSeries &aBars, const int aPeriod,
                         const size_t aMinTouch, const double aTollerance,
                         TPriceSeries &aoMin, TPriceSeries &aoMax) {
  ...
  for (size_t i = lPeriod; i < lDataSize; ++i) {
    ...
    TSimpleTick lTick{aBars[i].DateTime, lRollerLow.GetValue().min, 1.0};
...
bool _ForwardMinMax(const TBarSeries &aBars, const size_t aTimeDelta,
  ...
  for (size_t i = 0; i < lDataSize - 1; ++i) {
    ...
    while (j < lDataSize) {
      if (aBars[j].DateTime > (aBars[i].DateTime + ToDouble(aTimeDelta)))
        break;
    ...
    TSimpleTick lTick{aBars[i].DateTime, lLow, 1.0};
  ...
  TSimpleTick lTick{aBars[i].DateTime, GetBadPrice(), 0.0};
  aoMin[i] = lTick;
  aoMax[i] = lTick;
...
//------------------------------------------------------------------------------------------
TPriceSeries _KAMA(const TPriceSeries &aPrices, const int aPeriod,
                   const double aCoeff, const double aFastPeriod,
                   const double aSlowPeriod, const size_t aLag) {
  ...
  if (lResultSize > lPeriod) {
    ...
    for (size_t i = lPeriod; i < lResultSize; ++i) {
      const double lDirection =
          std::abs(aPrices[i].Price - aPrices[i - lPeriod].Price);

      lVolatility += std::abs(aPrices[i].Price - aPrices[i - 1].Price);
      if (i > lPeriod) {
        lVolatility -= std::abs(aPrices[i - lPeriod].Price -
                                aPrices[i - lPeriod - 1].Price);
      }

      const double lEfficiencyRatio =
          isZero(lVolatility) ? 1.0 : (lDirection / lVolatility);
      const double smooth =
          (lEfficiencyRatio * (lFastest - lSlowest) + lSlowest);
      const double c = std::pow(smooth, aCoeff);
      const TPrice lCurrentValue =
          c * aPrices[i].Price +
          (1.0 - c) *
              ((i > lPeriod) ? lResult[i - 1].Price : aPrices[i - 1].Price);
...
//------------------------------------------------------------------------------------------
TPriceSeries _IntradayParabolicSar(const TBarSeries &aBars, const double aAf,
                                   const double aMaxAf) {
  ...
  TSimpleTick lTick0{lFirstBar.DateTime, lFirstBar.Low, 1.0};
  lResult[0] = lTick0;
  ...
  for (size_t i = 1; i < aBars.size(); ++i) {
    ...
    if (not IsOneDay(aBars[i - 1].DateTime, aBars[i].DateTime)) {
      ...
      lAf = lInitaAfValue;
      TSimpleTick lTick{aBars[i].DateTime, aBars[i].Low, 1.0};
      lResult[i] = lTick;
      continue;
    }
    ...
    if (lCurrentSide == TDealSide::Buy) {
      ...
      TSimpleTick lTick{lCurrentBar.DateTime, lSar, 1.0};
      lResult[i] = lTick;
      continue;
    }
...
TPriceSeries _Stochastic(const TBarSeries &aBars, const int aPeriod) {
  ...
  if (_RollMinMax(aBars, aPeriod, lMin, lMax)) {
    ...
    for (size_t i = lPeriod; i < lDataSize; ++i) {
      ...
      lResult[i].Price =
          isZero(lRange) ? 50.0
                         : (100.0 * (aBars[i].Close - lMin[i].Price) / lRange);
      lResult[i].DateTime = aBars[i].DateTime;
...
//------------------------------------------------------------------------------------------
bool _BollingerBands(const TPriceSeries &aPrices, const int aPeriod,
                     const double aSigma, TPriceSeries &aoMin,
                     TPriceSeries &aoMean, TPriceSeries &aoMax) {
